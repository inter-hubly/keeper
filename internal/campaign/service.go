...
type campaignService struct {
	...
	variableRepository variables.Repository
...
func NewService(ctx context.Context) *campaignService {
	_campaignServiceOnce.Do(func() {
		_campaignService = &campaignService{
			campaignRepository: NewRepository(ctx),
			contactRepository:  contact.NewContact(ctx),
			variableRepository: variables.NewRepository(ctx),
			templateRepository: templates.NewRepository(ctx),
...
func (c *campaignService) StartCampaign(ctx context.Context, user *hctx.Logged, campaignId string) error {
	hlog.Debug(ctx, "campaign.service.StartCampaign", fmt.Sprintf("start campaign %s", user.Username))
	...
	if err != nil {
		hlog.Error(ctx, "campaign.service.StartCampaign", fmt.Sprintf("error marshalling campaign struct: %v", err))
		return err
	}
...
func (c *campaignService) GetCampaign(ctx context.Context, user *hctx.Logged) (*entity.Campaign, error) {
	hlog.Debug(ctx, "campaign.service.GetCampaign", fmt.Sprintf("found campaign for %s", user.Username))
	campaign, err := c.campaignRepository.GetCampaignById(ctx, user.Tenant)
	if err != nil {
		hlog.Error(ctx, "campaign.service.GetCampaign", fmt.Sprintf("error getting campaign by id: %v", err))
		return nil, err
	}
	return campaign, nil
}
...
func (c *campaignService) SaveCampaign(ctx context.Context, loggedUser *hctx.Logged, campaignDto *kdto.Campaign) (*entity.Campaign, error) {
	hlog.Debug(ctx, "campaign.service.SaveCampaign", fmt.Sprintf("%s saving campaign with dto %s", loggedUser.Username, campaignDto.Name))
	contacts, err := c.contactRepository.FindContacts(ctx, loggedUser.Tenant)
	if err != nil {
		hlog.Error(ctx, "campaign.service.SaveCampaign", fmt.Sprintf("Contact Repository Error: %v", err))
		return nil, err
	}
	if !c.containsAll(contacts, campaignDto.ContactsID) {
		hlog.Error(ctx, "campaign.service.SaveCampaign", "this phone does not contain contacts")
		return nil, errors.New("this phone does not contain contacts")
	}
	allVariables, err := c.variableRepository.GetVariables(ctx)
	if err != nil {
		hlog.Error(ctx, "campaign.service.SaveCampaign", fmt.Sprintf("VariableRepository Error: %v", err))
		return nil, err
	}
	if !c.containsAllVariables(allVariables, campaignDto.Variables) {
		hlog.Error(ctx, "campaign.service.SaveCampaign", "this phone does not contain variables")
	...
	if err != nil {
		hlog.Error(ctx, "campaign.service.SaveCampaign", fmt.Sprintf("Template Repository Error: %v", err))
		return nil, err
	}
...
func (c *campaignService) ListCampaign(ctx context.Context, user *hctx.Logged) ([]entity.Campaign, error) {
	hlog.Debug(ctx, "campaign.service.ListCampaign", fmt.Sprintf("list campaign %s", user.Username))
