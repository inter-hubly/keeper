...
func (r *userRepository) GetUserByUsername(ctx context.Context, userEmail string) (*entity.User, error) {
	hlog.Debug(ctx, "user.repository.GetUserByUsername", userEmail)
	...
	if err != nil {
		hlog.Error(ctx, "user.repository.GetUserByUsername", fmt.Sprintf("error find UserEmail %s : %s", userEmail, err))
		return nil, err
	}
	...
	if err = queryExec.Scan(
		...
		hlog.Error(ctx, "user.repository.GetUserByUsername", fmt.Sprintf("error scan UserEmail %s : %s", userEmail, err))
		return nil, err
	}
	if tenantID.Valid {
...
func (r *userRepository) SaveUser(ctx context.Context, user *entity.User) error {
	hlog.Debug(ctx, "user.repository.UpdateClientInUser", fmt.Sprintf("save User %+v", user))
	query := `
...
func (r *userRepository) UpdateClientInUser(ctx context.Context, tenantId, userId string) error {
	hlog.Debug(ctx, "user.repository.UpdateClientInUser", fmt.Sprintf(" Set tenantId: %s and UserId: %s", tenantId, userId))
	query := `UPDATE "user" SET tenant_id = $1 WHERE id = $2`
	if _, err := r.connection.Exec(query, tenantId, userId); err != nil {
		hlog.Error(ctx, "user.repository.UpdateClientInUser", fmt.Sprintf("update User Error : %s", err))
		return err
	}
	return nil
}
