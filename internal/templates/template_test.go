...
import (
	...
	gatewayMock "github.com/inter-hubly/keeper/internal/gateway/mocks"
...
func TestTemplateMediator(t *testing.T) {
	...
	for _, v := range []struct {
		testName string
		hasError bool
		template *domain2.Template
		auxFunc  func(template *domain2.Template)
	}{
		{
			...
			auxFunc: func(template *domain2.Template) {
				allMocks.whatsAppGateway.EXPECT().CreateTemplate(gomock.Any(), gomock.Any()).
					Do(func(_ context.Context, template *domain2.Template) {
...
func newDomainTemplate(bodyText string) *domain2.Template {
	headerText := "Lembrete de Pagamento"
	footerText := "See ya!"
	components := []domain2.Component{
...
func newVariables() *domain2.Variables {
	return &domain2.Variables{
		Variable: map[string]domain2.SingleVariable{
			"name": {
				Slug:  "name",
				Label: "name",
			},
