...
import (
	...
	gateway2 "github.com/inter-hubly/keeper/internal/gateway"
...
type templateMediator struct {
	...
	whatsAppGateway     gateway2.WhatsApp
}

func NewMediator(ctx context.Context) *templateMediator {
	_templateMediatorOnce.Do(func() {
		env := server.GetEnvironment().Env
		var gtw gateway2.WhatsApp
		if env == server.Development {
			gtw = gateway2.NewWhatsAppMock()
		} else {
			gtw = gateway2.NewWhatsApp(ctx)
		}
		_template = &templateMediator{
