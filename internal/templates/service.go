...
import (
	...
	"github.com/inter-hubly/keeper/internal/gateway"
...
type Service interface {
	...
	SaveVariable(ctx context.Context, variables []kdto2.Variable, templateId string) error
...
func NewService(ctx context.Context) *templateService {
	_templateServiceOnce.Do(func() {
		_templateService = &templateService{
			templateRepository: NewRepository(ctx),
...
func (s *templateService) SincronizeWhatsAppTemplate(ctx context.Context, user *hctx.Logged) error {
	hlog.Debug(ctx, "template.service.SincronizeWhatsAppTemplate", fmt.Sprintf("Sincronize %s", user.UserId))
	allTemplates, err := s.whatsAppGateway.FindAllTemplate(ctx)
	if err != nil {
		hlog.Error(ctx, "template.service.SincronizeWhatsAppTemplate", err.Error())
		return err
	}
...
func normalizeComponent(components []kdto2.ComponentDto) []domain.Component {
...
func (s *templateService) SaveVariable(ctx context.Context, variables []kdto2.Variable, templateId string) error {
	hlog.Debug(ctx, "template.service.SaveVariable", fmt.Sprintf("templateId: %s", templateId))
	template, err := s.templateRepository.GetTemplateById(ctx, templateId)
	if err != nil {
		hlog.Error(ctx, "template.service.SaveVariable", err.Error())
		return err
	}
...
func (s *templateService) SearchTemplates(ctx context.Context, user *hctx.Logged) ([]domain.Template, error) {
	hlog.Debug(ctx, "template.service.FindAll", fmt.Sprintf("Find All Templates %s", user.UserId))
...
func (s *templateService) CountVariables(ctx context.Context, templateId string) (int, error) {
	hlog.Debug(ctx, "template.service.CountVariables", fmt.Sprintf("templateId: %s", templateId))
	template, err := s.templateRepository.GetTemplateById(ctx, templateId)
	if err != nil {
		hlog.Error(ctx, "template.service.CountVariables", err.Error())
		return 0, err
	}
